...
def import_object(model_path, rot_mat=((1, 0, 0), (0, 1, 0), (0, 0, 1)), trans_vec=(0, 0, 0), scale=1, name=None, debug_mode=False):
    """
    ...
    for i, obj in enumerate(bpy.context.selected_objects):
        ...
        obj_list.append(obj)
        print(obj.mode, "MODE OF OBJ")
        for f in obj.data.polygons:
            f_list.append(1)

        for v in obj.data.vertices:
            v_list.append(tuple(v.co))
        # obj.data.transform(obj.matrix_world)
        for v_new in obj.data.vertices:
            v_list_new.append(tuple(v_new.co))
    # Scene update necessary, as matrix_world is updated lazily
    # bpy.context.scene.update()
    ...
    if debug_mode:
        return np.mean(v_list, axis=0), np.min(v_list, axis=0), np.max(v_list, axis=0), np.mean(v_list_new, axis=0), np.min(v_list_new, axis=0), np.max(v_list_new, axis=0), v_list.shape[0], len(f_list)
    if len(obj_list) == 1:
        return obj_list[0]
    else:
        return obj_list

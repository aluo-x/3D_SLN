...
def render_mask(outpath, cam=None, obj_names=None):
    ...
    world.use_nodes = True
    try:
        # world.horizon.backgroundColor = [0,0,0]
        world.node_tree.nodes.remove(world.node_tree.nodes['Background'])
    except KeyError:
        pass
    import time
    ...
    for obj in bpy.data.objects:
        usethis = False
        for valid_name in obj_names:
            if valid_name.lower() in str(obj.name.lower()):
                if obj.type == 'MESH':
                    usethis = True
        if usethis:
            obj.hide_render = False
            ...
            for i_mat in range(len(obj.material_slots)):
                bpy.ops.object.material_slot_remove({'object': obj})

            bpy.ops.object.material_slot_add({'object': obj})
            obj.data.materials[0] = bpy.data.materials.new("My Mat")
            ...
            # node_tree.links.new(nodes['Emission'].outputs[0],
            #                     nodes['Material Output'].inputs[1])
        else:
...
def render_depth(outpath, cam=None, obj_names=None, ray_depth=False):
    ...
    nodes = node_tree.nodes
    # print(len(nodes['Render Layers'].outputs[0].links), "LINKS")
    ...
    # node_tree.links.new(nodes['Render Layers'].outputs[4], id_mask.inputs[0])


    # node_tree.links.new(nodes['Render Layers'].outputs[3], nodes['File Output'].inputs[0])
...
def render_mask_id(outpath, cam=None, obj_names=None, ray_depth=False):
    """
    Render binary masks of objects from the specified camera
